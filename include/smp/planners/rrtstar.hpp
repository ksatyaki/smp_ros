...
template <class typeparams> smp::rrtstar<typeparams>::rrtstar() {
...
template <class typeparams>
int smp::rrtstar<typeparams>::initialize(state_t *initial_state_in) {
...
template <class typeparams>
int smp::rrtstar<typeparams>::init_cost_evaluator(
    cost_evaluator_t &cost_evaluator_in) {
...
int smp::rrtstar<typeparams>::propagate_cost(vertex_t *vertex_in,
  ...
  for (typename std::list<edge_t *>::iterator iter_edge =
    ...
    if (vertex_next != vertex_in)
      this->propagate_cost(
          vertex_next, vertex_in->data.total_cost + edge_curr->data.edge_cost);
  }
  ...
  if (this->extender.extend(vertex_nearest->state, state_sample,
    ...
    if (check_extended_trajectory_for_collision(vertex_nearest->state,
      ...
      if (parameters.get_phase() >= 1) { // Check whether phase 1 should occur.
        ...
        state_extended =
            new state_t(*(trajectory_parent->list_states
